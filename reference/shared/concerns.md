...
- **Prefer composition** — consider strategies, form objects, query objects etc. before concerns
...
| Cross-cutting callbacks | Form objects etc |
...
class SoftDeletableTest < ActiveSupport::TestCase
  ...
  test ".kept excludes soft-deleted records" do
    record = model_class.create!(title: "Test")
    record.soft_delete

    assert_not_includes model_class.kept, record
  end
end
```
...
# GOOD: Keep in model or extract to composition
class Post < ApplicationRecord
  after_create :do_thing_1
  # Or use composition
end
...
- [ ] Could we use composition instead?
