...
**This is an edge case pattern.** Most Rails apps do fine with `Rails.application.credentials`
or `ENV.fetch("KEY")`. Reach for structured config objects when:

- **10+ settings across 3+ external services** — Stripe + SendGrid + Twilio each with
  3-4 keys; grouping catches missing keys at boot instead of at runtime
- **Multi-tenant SaaS** — per-tenant API keys, limits, or feature flags
- **Self-hosted / operator-configured apps** — dozens of YAML settings with
  environment-specific overrides (Discourse/GitLab style)
- **Configurable business rules** — pricing tiers, rate limits, retry policies that
  vary per environment but aren't feature flags

**When NOT to use:** a typical Rails app with 5-6 ENV vars — `ENV.fetch` or
`Rails.application.credentials` is enough.
...
class PaymentProcessorTest < ActiveSupport::TestCase
  def setup
    ...
    @processor = PaymentProcessor.new(config: @config)
  end

  test "uses configured currency" do
    stub = ->(args) { assert_equal "eur", args[:currency] }
    Stripe::Charge.stub(:create, stub) do
...
class StripeConfigTest < ActiveSupport::TestCase
  test "uses environment variable override" do
    ClimateControl.modify(STRIPE_CURRENCY: "gbp") do
      config = StripeConfig.new
      assert_equal "gbp", config.currency
    end
  end

  test "requires api_key" do
    assert_raises(Anyway::Config::ValidationError) do
      StripeConfig.new(webhook_secret: "test")
    end
  end
end
```
